...
const PATTERNS = {
  critical: [
    ...
    },
    {
      ...
      patterns: [
        // Exclude known-good scripts from this check
        /^(?!.*\/(?:auth_check\.sh|skulk-email\.sh|moltbook-check\.sh)$).*(?:cat|read|access|steal|copy|send|forward|exfil).*(?:\.ssh|authorized_keys|id_rsa|id_ed25519|\.env|credentials|secrets?\.|api[_-]?key|token|password)/gim,
  ...
  high: [
    ...
    },
    {
      ...
      patterns: [
        ...
        // Exclude known-good scripts that use process.env intentionally
        /^(?!.*\/(?:auth_check\.sh|skulk-email\.sh|moltbook\.js)$).*process\.env(?:\[|\.)/gim,
...
function scanSkill(skillPath, options = {}) {
  ...
  const finalFindings = results.findings.filter(
    (f) => !options.ignoreRules?.includes(f.ruleId)
  );
  ...
  if (finalFindings.some((f) => f.severity === 'critical')) {
...
for (let i = 0; i < args.length; i++) {
  ...
  if (args[i] === '--ignore-rule' && args[i + 1]) {
    ignoreRules.push(args[i + 1]);
    i++;
  }
}
...
Usage: node scanner.js <path-to-skill> [--json] [--summary] [--verbose] [--include-self] [--ignore <path>] [--ignore-rule <ruleId>]
...
Examples:
  ...
  node scanner.js ./skills/my-skill --ignore-rule credential-access
  node scanner.js ./skills/skulk-skill-scanner --include-self
`);
  process.exit(0);
}

const results = scanSkill(path.resolve(skillPath), { includeSelf, ignore, ignoreRules });
